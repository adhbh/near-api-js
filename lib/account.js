...
class Account {
     ...
     */
    async signTransaction(receiverId, actions, extNonce) {
    ...
    async signAndSendTransactionV2({ receiverId, actions, returnError, nonce }) {
        let txHash, signedTx;
        ...
        const result = await exponential_backoff_1.default(TX_NONCE_RETRY_WAIT, TX_NONCE_RETRY_NUMBER, TX_NONCE_RETRY_WAIT_BACKOFF, async () => {
            ...
            try {
                ...
                error.context = new providers_1.ErrorContext(borsh_1.baseEncode(txHash), error.type);
                throw error;
            }
        });
        if (!result) {
    ...
    functionCallV2({ contractId, methodName, args = {}, gas = constants_1.DEFAULT_FUNCTION_CALL_GAS, attachedDeposit, walletMeta, walletCallbackUrl, stringify, jsContract, nonce }) {
        this.validateArgs(args);
